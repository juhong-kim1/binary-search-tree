...
public enum TileTypes //맵 타입들이랑 몬스터
{
    Empty = -1,
    // 0, 14
    Grass = 15,
...
{ 
    ...
    public Tile[] CoastTiles
    {
        ...
        get
        {
            return tiles.Where(t => t.autoTileId < (int)TileTypes.Grass).ToArray(); // 그래스 아래있는 타일들
        }
    } 
    ...
    public Tile[] LandTiles
    {
        get
        {
            return tiles.Where(t => t.autoTileId >= (int)TileTypes.Grass).ToArray(); // 그래스 위에있는 타일들
        }
    }

    public void Init(int rows, int cols)   // 0: O 1: X       타일 배치 및 힌계조정, id 할당
    {
        this.rows = rows;
        this.cols = cols;
        ...
        for (int i = 0; i < tiles.Length; i++)
        {
            tiles[i].UpdateAutoTileId(); //타일 id 업데이트
            tiles[i].UpdateAutoFowId(); // fow id 업데이트
        }
    }
        ...
        float monsterPercent)
    {
        ...
        int randomCastle = Random.Range(1, towns.Length);
        var endTile = towns[randomCastle].autoTileId = (int)TileTypes.Castle;

        var search = new GraphSearch();
        search.Init(this);
        if (!search.AStar(startTile, towns[randomCastle]))
        {
            return false;
        }
